 ...
 * Elle permet d'afficher l'état du jeu et de récupérer les décisions des
 * joueurs,
 * qu'ils soient humains (saisie clavier) ou ordinateurs (affichage des choix
 * auto).
 */
public class UserInterface {
    ...
    public boolean askForInsurance(boolean isHuman, double betInsur, boolean ansComputer) {
        return askForYesOrNo(isHuman, "V\nVeux-tu t'assurer pour le quart de ta mise, soit " + betInsur
                + " € contre un Black Jack du croupier", ansComputer);
    }

    public boolean askForSurrender(boolean isHuman, double bet, boolean ansComputer) {
        return askForYesOrNo(isHuman,
                "\nVeux-tu abandonner ce tour et récupérer la moitié de ta mise, soit " + bet / 2 + " € ?",
                ansComputer);
    }
    ...
    private double readDoubleBetween(String prompt, double min, double max) {
        ...
        do {
            ...
            this.input.nextLine(); // Consomme le retour chariot
            if (value < min || value > max) {
                this.output.println("Erreur tu dois entrer un nombre supérieur ou égal à " + min
                        + " et inférieur ou égal à " + max);
            }
        } while (value < min || value > max);
        return value;
    }
    ...
    /**
     * Action : Affiche la mise décidée par l'ordinateur. si this.displayRounds =
     * true
     */
    ...
    public double askForBet(boolean isHuman, int playerNum, double balance, double ansComputer) {
        if (isHuman) {
            return this.readDoubleBetween("\nJoueur " + playerNum + " - solde " + balance + " - quelle est ta mise",
                    0.0, balance);
        } else {
            this.displayMessage("\nJoueur " + playerNum + " - solde " + balance + " - quelle est ta mise ? (0.0 à "
                    + balance + ") ");
    ...
    /**
     * Affiche la carte piochée par le croupier pendant son tour de jeu, ainsi que
     * son nouveau score
     */
    ...
    public void displayPlayerResult(double gain, double gainInsur, double balance, double bet, boolean insurance) {
        String message;
        if (gain == bet)
            message = "Égalité : tu récupères ta mise";
        else if (gain > 0)
            message = "Gagné ! Tu remportes " + gain + " €.";
        else
            message = "Perdu : tu perds ta mise !";
        this.displayMessage(message);

        if (insurance) {
            this.displayMessage("Tu t'es assuré contre un Black Jack du croupier.");
            if (gainInsur > 0) {
                this.displayMessage("Le croupier a fait un Blackjack, tu récupères 2 fois ta prime d'assurance, soit "
                        + gainInsur + " €");
            } else {
